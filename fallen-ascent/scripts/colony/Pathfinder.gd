...
# Cells that are water/acid family mapped to their respective float weights.
...
func _on_tile_changed(grid: Vector2i, new_tile: int) -> void:
	...
	var weight := 1.0
	if TileVisuals.is_acid_family(new_tile):
		weight = ACID_AVOID_FACTOR
		_fluid_cache[grid] = weight
	elif TileVisuals.is_water_or_acid_family(new_tile):
		weight = WATER_AVOID_FACTOR
		_fluid_cache[grid] = weight
	else:
		_fluid_cache.erase(grid)
		
	_astar.set_point_weight_scale(grid, weight)
	_astar_with_teleport.set_point_weight_scale(grid, weight)
...
func _rebuild() -> void:
	...
	if grew:
		...
		# update() wiped solidness and weights — replay every cell we've ever computed.
		# Bounded by total cells loaded over the session, not the region area.
		# Only executed when region expands to prevent frame spikes on standard chunk updates.
		for g in _solid_cache:
			var s: bool = _solid_cache[g]
			var teleport_here: bool = _teleporter_cache.has(g)
			var weight: float = _fluid_cache.get(g, 1.0)
			
			_astar.set_point_solid(g, s or teleport_here)
			_astar.set_point_weight_scale(g, weight)
			
			_astar_with_teleport.set_point_solid(g, s)
			_astar_with_teleport.set_point_weight_scale(g, weight)
	...
	for coord in _dirty_chunks:
		...
		for ly in Chunk.SIZE:
			for lx in Chunk.SIZE:
				...
				var weight := 1.0
				if TileVisuals.is_acid_family(tile):
					weight = ACID_AVOID_FACTOR
					_fluid_cache[g] = weight
				elif TileVisuals.is_water_or_acid_family(tile):
					weight = WATER_AVOID_FACTOR
					_fluid_cache[g] = weight
				else:
...
#
# When the natural shortest path crosses water or acid, the A* algorithm
# routes around it automatically if a suitable detour exists based on
# the associated weight scales.
func find_path(from: Vector2i, to: Vector2i) -> PackedVector2Array:
	...
	var unmark_to: bool = _teleporter_cache.has(to) and _astar.is_point_solid(to)
	if unmark_to:
...
# path: PackedVector2Array, entry: Vector2i, exit: Vector2i, uses_teleporter: bool.
func find_path_with_teleporters(from: Vector2i, to: Vector2i, fog: Object = null) -> Dictionary:
	...
	# Pre-calculate and cache paths from all potential exits to 'to'.
	# This cuts operations from O(N^2) pathfinding queries down to O(N).
	...
	for exit in teleporters:
		...
		if not _path_allowed_by_fog(path_from_exit, fog):
			continue
		exit_paths[exit] = path_from_exit
	...
	for entry in teleporters:
		...
		for exit in teleporters:
			if exit == entry:
				continue
			if not exit_paths.has(exit):
				continue
				
			var path_from_exit: PackedVector2Array = exit_paths[exit]
...
func _path_allowed_by_fog(path: PackedVector2Array, fog: Object) -> bool:
	...
	for waypoint in path:
		var grid := Vector2i((waypoint / float(Chunk.TILE_PIXELS)).floor())
